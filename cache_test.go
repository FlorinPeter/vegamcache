...
func TestMerges(t *testing.T) {
	...
	completedData := c.mergeComplete(map[string]Value{"bar": Value{Data: "foo", Expiry: time.Now().Add(5 * time.Second).UnixNano()}})
	...
	recivedData := c.mergeRecived(map[string]Value{"bar": Value{Data: "foo", Expiry: time.Now().Add(5 * time.Second).UnixNano(), LastWrite: time.Now().UnixNano()}})
